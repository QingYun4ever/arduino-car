...
class SelfLearningApp(AIBase):
    ...
    def __init__(self, kmodel_path, model_input_size, labels, top_k, threshold, database_path,
        ...
        self.features = [5] * len(self.labels)
...
def exce_demo(pl, recong_only=None):
    """
    自学习例程主要流程：
      1. 初始化自学习实例
    ...
    参数:
      ...
      recong_only (bool|None): True=仅识别；False=强制采集；None=自动判断(特征库已满则直接识别) // True=recognition only; False=force collection; None=auto (skip collection if database is complete)
    """
    ...
    # 自动判断模式：若特征库已存在且每类特征数量足够，则直接进入识别
    # Auto mode: if the feature database already exists with enough features per category, go straight to recognition
    if recong_only is None:
        recong_only = False
        try:
            if os.stat(database_path)[0] & 0x4000:
                file_names = os.listdir(database_path)
                counts = {}
                for f in file_names:
                    key = f.split("_")[0]
                    counts[key] = counts.get(key, 0) + 1
                recong_only = all(counts.get(label, 0) >= 5 for label in labels)
        except Exception:
            pass
...
# 退出例程，反初始化资源（保留特征库文件，供下次直接识别）
# Exit routine: deinitialize resources (keep feature files for next run)
def exit_demo():
    global sl
    # 执行自学习实例的反初始化，释放资源
...
if __name__ == "__main__":
    ...
    pl.create()
    # 执行自学习例程，None表示自动判断：特征库已存在则直接识别，否则先采集
    # Execute the self-learning demo. None means auto mode: recognize directly if the feature database exists, otherwise collect first.
    exce_demo(pl, None)
