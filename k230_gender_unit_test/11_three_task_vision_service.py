...
检测到非00状态后等待20ms再采样，避免11状态因两个Arduino digitalWrite
相差数微秒而被误判成01或10。任务完成后必须回到00才能再次触发。
...
def main():
    ...
    try:
        ...
        draw_status(pipeline, "00 IDLE 01 GENDER 10 FIRE 11 SHAPE")
        print("Three-task vision service started")
        print("Task bits [IO32 IO33]:")
        print("  00 idle")
        print("  01 gender")
        print("  10 fire color")
        print("  11 wafer shape")
        print("UART1 TX: IO%d baud=%d" % (
            UART_TX_PIN,
            UART_BAUDRATE,
        ))
        send_line(uart, "K230_VISION3_READY")
        ...
        print("Three-task vision service stopped by user")

    except Exception as error:
        if "IDE interrupt" in str(error):
            print("Three-task vision service stopped by CanMV IDE")
        else:
            print("Three-task vision service error:", error)
            raise
