"""CanMV K230由IO33高电平触发人脸性别识别。

信号流程：
- Arduino D11经5V→3.3V分压后向K230 IO33发送高电平脉冲
- K230发送 BUSY\n
- K230识别后通过IO9/UART1_TX发送 RESULT,M|F|U,<confidence_percent>\n

摄像头与模型只初始化一次。空闲时等待IO33上升沿；触发后对最大人脸
进行多帧投票，再通过单向UART把稳定结果发给Arduino。
...
def main():
    ...
    try:
        ...
        draw_status(pipeline, "IO33 READY")
        print("Face gender IO33 trigger service started")
        print("Trigger input: IO%d" % TRIGGER_PIN)
        print("UART1 TX: IO%d baud=%d" % (
            UART_TX_PIN,
            UART_BAUDRATE,
        ))
        send_line(uart, "K230_READY")
        ...
        while True:
            ...
            if trigger_level and trigger_armed:
                ...
                send_line(uart, "BUSY")
                draw_status(pipeline, "MEASURING")
        ...
        print("Face gender UART service stopped by user")

    except Exception as error:
        print("Face gender UART service error:", error)
        raise
