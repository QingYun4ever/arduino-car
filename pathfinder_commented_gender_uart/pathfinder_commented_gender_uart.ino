/*
 * ============================================================
 * pathfinder_commented_gender_uart.ino —— 路线+K230性别识别实验副本
 * ============================================================
 *
 * 【本文件说明】
 *   - 本文件复制自 pathfinder_commented/pathfinder_commented.ino。
 *   - 仅用于测试 IO33触发+K230单向UART返回，不替代已验证的路线原件。
 *   - 第4、5、6段的视觉调用点是候选位置，必须根据实车朝向确认。
 *
 ...
 *   ├──────────┼──────────┼──────────────────────────┤
 *   │ D8       │ 蜂鸣器    │ 低电平有效(LOW=响)          │
 *   │ D11      │ 视觉触发  │ 经5V→3.3V分压后接K230 IO33    │
...
/* ---- K230视觉触发与单向UART返回 ----
 * Nano D11输出高电平脉冲，经5V→3.3V分压后接K230 IO33。
 * K230 IO9/UART1_TXD单向发送结果到Nano D12/RX。
 * SoftwareSerial仍需一个TX参数，A5仅作未接线的占位引脚。
 * 注意：此实验副本中D11/D12不再用于RGB绿灯/红灯。 */
...
const unsigned long K230_RESPONSE_TIMEOUT_MS = 3500;
const unsigned long K230_TRIGGER_HIGH_MS = 100;
...
void getThresholds(bool forward, int initSpeed, int straightCorrection) {
  ...
  for (int i = 1; i <= 3; i = i + (1)) {  // D12用于UART，改由蓝灯提示白场采样完成
    digitalWrite(13, LOW);
    delay(75);
  ...
  for (int i = 1; i <= 3; i = i + (1)) {  // D12用于UART，改由蓝灯提示黑线采样完成
    digitalWrite(13, LOW);
    delay(75);
    digitalWrite(13, HIGH);
    delay(75);
 ...
 *   K230通过IO9→Nano D12单向回复 BUSY\n，再回复
 *   RESULT,M|F|U,<confidence>\n。
 *   M=男性，F=女性，U=无法稳定判断。
...
GenderResult requestGenderOnce(uint8_t zone, uint8_t *confidence) {
  ...
  while (millis() - startedAt < K230_RESPONSE_TIMEOUT_MS) {
    while (k230Serial.available() > 0) {
...
/* ============================================================
 * setup: 初始化所有引脚与变量
 *   - D11为K230 IO33触发输出，D12为SoftwareSerial接收
 *   - 仅保留D13蓝灯；蜂鸣器D8、舵机D9初始化
...
void setup() {
  ...
  pinMode(K230_TRIGGER_PIN, OUTPUT);            // K230 IO33触发线
  digitalWrite(K230_TRIGGER_PIN, LOW);          // 默认不触发
  // D12由SoftwareSerial接收；D11触发，均不再用于RGB。
